...
def POS(xp, x):
    ...
    sTx = float(k[3])
    sTy = float(k[7])
    s = float((np.linalg.norm(R1) + np.linalg.norm(R2)) / 2)
    t = np.array([sTx, sTy], dtype=np.float32)
...
def resize_n_crop_img(img, lm, t, s, target_size=224., mask=None):
    w0, h0 = img.size
    w = int(round(w0 * float(s)))
    h = int(round(h0 * float(s)))
...
def align_img(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    ...
    trans_params = np.array([float(w0), float(h0), float(s), float(t[0]), float(t[1])], dtype=np.float32)
